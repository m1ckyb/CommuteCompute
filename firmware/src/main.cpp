...
// DEFAULT_SERVER removed - turnkey requirement (URL comes from pairing only)
...
#define BLE_CHAR_SERVER_UUID    "CC000004-0000-1000-8000-00805F9B34FB"  // Server URL (turnkey per Section 17.4)
...
char serverUrl[256] = "";    // User server URL (turnkey - no hardcoded default)
char pairingCode[8] = "";
...
#define FACTORY_RESET_HOLD_MS 5000  // Hold button for 5 seconds
#define FACTORY_RESET_PIN PIN_INTERRUPT
...
void performFactoryReset() {
    ...
    // Erase NVS completely
    nvs_flash_erase();
    nvs_flash_init();
    
    Serial.println("[RESET] NVS erased. Rebooting...");
    delay(500);
    ESP.restart();
}
...
bool checkFactoryResetButton() {
    ...
    // Check if button is being held (LOW = pressed)
    if (digitalRead(FACTORY_RESET_PIN) == LOW) {
        Serial.println("[RESET] Button held - checking for factory reset...");
        ...
        bbep->setCursor(centerX - 80, centerY - 40);
        bbep->print("FACTORY RESET");
        bbep->setCursor(centerX - 100, centerY);
        bbep->print("Hold button for 5 seconds");
        bbep->setCursor(centerX - 80, centerY + 40);
        bbep->print("Release to cancel");
        bbep->refresh(REFRESH_FULL, true);
        
        // Wait and check if still held after 5 seconds
        unsigned long startTime = millis();
        while (digitalRead(FACTORY_RESET_PIN) == LOW) {
            if (millis() - startTime >= FACTORY_RESET_HOLD_MS) {
                // Show confirmation
                bbep->fillScreen(BBEP_WHITE);
                bbep->setCursor(centerX - 60, centerY);
                bbep->print("WIPING...");
                bbep->refresh(REFRESH_FULL, true);
                delay(500);
                
                performFactoryReset();
                return true;  // Won't reach here (reboots)
            }
            delay(100);
        }
    ...
    void onWrite(BLECharacteristic* pChar) {
        ...
        if (value.length() > 0) {
            if (uuid.indexOf("0002") > 0) {
                ...
                // Server URL received (turnkey per Section 17.4)
...
void loop() {
    ...
    switch (currentState) {
        // ==== BOOT: Show logo ====
        case STATE_BOOT: {
            // Check for factory reset (hold button during boot)
            if (checkFactoryResetButton()) {
                return; // Will reboot after reset
            }
...
void initBLE() {
    ...
    // Server URL characteristic (turnkey per Section 17.4)
    BLECharacteristic* pCharServer = pService->createCharacteristic(BLE_CHAR_SERVER_UUID, BLECharacteristic::PROPERTY_WRITE);
    pCharServer->setCallbacks(new CredentialCallbacks());
...
bool pollPairingServer() {
    ...
    // Turnkey: Use user-provided server URL (Section 17.4)
    if (strlen(serverUrl) == 0) {
        Serial.println("[PAIR] No server URL configured - cannot poll");
        return false;
    }
    String url = String(serverUrl) + "/api/pair/" + String(pairingCode);
...
void showPairedScreen() {
    ...
    // Footer
    bbep->drawLine(50, 440, 750, 440, BBEP_BLACK);
    bbep->setCursor(220, 455); bbep->print("(c) 2026 Angus Bergman - AGPL v3");
...
void loadSettings() {
    ...
    String server = preferences.getString("serverUrl", "");
    ...
    strncpy(serverUrl, server.c_str(), sizeof(serverUrl) - 1);

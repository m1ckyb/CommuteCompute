# CCDashDesignV12 - Commute Compute Dashboard Specification

**Status:** 🔒 LOCKED — DO NOT MODIFY WITHOUT EXPLICIT APPROVAL  
**Version:** 12.0  
**Locked Date:** 2026-02-01  
**Effective Date:** 2026-02-01 (Melbourne Metro Tunnel Launch)  
**Display:** 800×480px e-ink (TRMNL device)  
**Refresh:** 60-second partial refresh cycle  
**Renderer:** ccdash-renderer.js v1.40  
**SmartCommute:** v2.0 (Metro Tunnel Compliant)
...
CCDash V12 is fully compliant with the Melbourne Metro Tunnel network restructure. The SmartCommute™ engine automatically handles routing for the five lines that now use the Metro Tunnel instead of the City Loop.
...
When a user's journey involves a discontinued service (e.g., Pakenham Line at Southern Cross), the SmartCommute engine provides alternatives. The renderer does NOT display discontinued routes.
...
- **Format:** `+X min DELAY` (inverted, white text on black)
- **Threshold:** Display when delay ≥ 1 minute

### 9.2 Status Bar with Delay

```
│ DELAY → Arrive 9:10am (+25 min)              [+25 min DELAY]     55 min    │
```

### 9.3 Delayed Leg Styling

- **Border:** 2px black border around entire leg box
- **Subtitle:** `+X MIN • Next: A, B min`

### 9.4 Multiple Delays

When multiple legs are delayed:
- Each delayed leg gets border
- Status bar shows cumulative delay
- Badge shows total delay
